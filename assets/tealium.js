...
if ((the_url.indexOf("/articles") != -1) || (the_url.indexOf("/sections") != -1)) {  //check if customer is on a section or article as they have breadcrumbs
    var bread_crumbs = document.querySelector('ol[class="breadcrumbs"]'); // get the breadcrumbs
    if (bread_crumbs) { // if page has breadcrumbs
    ...
    var page_id = "" // i.e 360053451451
    var homepage = document.getElementById('home-section');  // get home page (unique id)
    var user_id= "";
    var page_category_detail = " "; // i.e sports, navigation, wi-fi etc
...
if (troubleshooting) {
    ...
    if (language.match(/^(en-ca|en-us|fr-ca|pt-br|es-ar|es-mx)$/)) {
        region = "AMER" 
    } else if (language.match(/^(nl-nl|de-de|fr-fr|it-it|nl-be|en-ie|en-za|en-gb|fi|fr|fr-be|fr-lu|fr-ch|de|de-at|de-ch|el|hu|it|no|nb|pl|pt|es|sv)$/)) {
        region = "EMEA" 
